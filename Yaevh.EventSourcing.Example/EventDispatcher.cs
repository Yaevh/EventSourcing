...
public static class EventDispatcher
{
    ...
    /// <typeparam name="TAggregate">The type of the aggregate associated with the events.</typeparam>
    /// <typeparam name="TAggregateId">The type of the aggregate identifier.</typeparam>
    /// <param name="aggregate">The aggregate instance associated with the events.</param>
    ...
    public static async Task DispatchEvents<TAggregate, TAggregateId>(
        ...
        where TAggregateId : notnull
    {
        ...
        {
            var eventHandlerType = typeof(IAggregateEventHandler<,,>).MakeGenericType(typeof(TAggregate), typeof(TAggregateId), @event.Payload.GetType());
            ...
            foreach (var handler in handlers)
                await (Task)method.Invoke(handler, [aggregate, @event.Payload, cancellationToken])!;
        }
    }
    ...
    /// <typeparam name="TAggregate">The type of the aggregate associated with the event.</typeparam>
    /// <typeparam name="TAggregateId">The type of the aggregate identifier.</typeparam>
    /// <param name="aggregate">The aggregate instance that the event is associated with. Cannot be <see langword="null"/>.</param>
    ...
    /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
    /// <returns>A task that represents the asynchronous operation.</returns>
    public static async Task DispatchEvent<TAggregate, TAggregateId>(
        TAggregate aggregate,
        ...
        where TAggregateId : notnull
    {
        await DispatchEvents(aggregate, new[] { @event }, serviceProvider, cancellationToken);
    }
}
